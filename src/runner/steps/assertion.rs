...
use crate::types::{
    ...
    PatuiEvent, PatuiEventKind, PatuiExpr, PatuiStepAssertion, PatuiStepData, PatuiStepDataFlavour,
};
        ...
        let task = tokio::spawn(async move {
            ...
            while let Some(_) = notify_rx.recv().await {
                ...
                match eval(&expr, &results) {
                    ...
                    Ok(EvalResult::Predictable(patui_step_data)) => todo!(),
            ...
            for (expr, mut receiver) in receivers.into_iter() {
                ...
                tasks.push(tokio::spawn(async move {
                    ...
                    while let Ok(data) = receiver.recv().await {
                        ...
                        let mut entry = lock.entry(expr.clone()).or_insert(vec![]);
                        entry.push(data);
                        drop(entry);
                        drop(lock);
...
mod tests {
    ...
    async fn single_channel_read_and_eval_null() {
        ...
        input_tx.send(PatuiStepData::new(PatuiStepDataFlavour::Null));
